...
def load_archive(filename: str):
    ...
    entry_archive = EntryArchive.m_from_dict(json.load(open(filename)))
    run_normalize(entry_archive)
...
#     assert len(tasks) == len(entry_archive.run[0].calculation)
#     assert (
#         tasks[0].inputs[0].section.m_proxy_resolve() == entry_archive.run[0].method[0]
#     )
#     assert tasks[-1].outputs[0].section == entry_archive.run[0].calculation[-1]
...
def test_eos_workflow():
    ...
    print(archive.workflow2.tasks)
    print(archive.workflow2.results.eos_fit)
