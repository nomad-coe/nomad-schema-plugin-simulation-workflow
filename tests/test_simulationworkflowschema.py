...
def test_elastic_workflow():
    elastic_archive = run_parsing(
        ElasticParser, "tests/data/elastic/INFO_ElaStic")
...
def test_phonon_workflow():
    phonopy_archive = run_parsing(
        PhonopyParser,
        'tests/data/phonopy/phonopy-FHI-aims-displacement-01/control.in')
...
def test_rdf_2():
    archive = run_parsing(
        GromacsParser, 'tests/data/gromacs/fe_test/mdrun.out')
