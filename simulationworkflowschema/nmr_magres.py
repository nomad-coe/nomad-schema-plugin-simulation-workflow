...
# TODO extend for the first principles NMR case (which will group DFT and NMR)
class NMRMagResResults(NMROutputs):
    """
    Groups NMR and the NMR magres outputs: band gaps, DOS, band structures, and magnetic
    outputs. The ResultsNormalizer takes care of adding a label 'NMR' or 'NMR MagRes' in
    the method `get_nmr_magres_workflow_properties`.
    """
...
class NMRMagResMethod(SimulationWorkflowMethod):
    """
    Groups NMR and the NMR magres input methodologies.
    """
...
class NMRMagRes(SimulationWorkflow):
    """
